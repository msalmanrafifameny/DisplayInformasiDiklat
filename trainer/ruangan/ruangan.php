...
<body>
                    ...
                    <div class="row mb-2">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <?php
                                $no = 1;
                                $sqld = "SELECT * FROM ruangan";
                                $resultd = mysqli_query($conn, $sqld);

                                if ($resultd) {
                                    while ($rowd = mysqli_fetch_assoc($resultd)) {
                                        $id_ruangan = $rowd['id_ruang'];
                                        $nama_ruangan = $rowd['nama_ruangan'];
                                        $ketersediaan = $rowd['ketersediaan'];
                                ?>
                                        <tr>
                                            <td>
                                                <?php echo $no++; ?>
                                            ...
                                            <td>
                                                <?php
                                                if ($ketersediaan == 0) {
                                                    echo '<div class="badge bg-success">Tersedia</div>';
                                                } elseif ($ketersediaan == 1) {
                                                    echo '<div class="badge bg-danger">Tidak Tersedia</div>';
                                                }
                                                ?>
                                            </td>
                                            <td class="text-center">
                                                <a class="btn btn-warning" href="edit.php?id_ruang=<?php echo $id_ruangan; ?>">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <a class="btn btn-danger" href="#" onclick="confirmDelete(<?php echo $id_ruangan; ?>)">
    ...
    <script>
        function confirmDelete(id_ruang) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "hapus.php?id_ruang=" + id_ruang;
                } else {
                    Swal.fire({
